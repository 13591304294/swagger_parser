...
class OpenApiParser {
  ...
  OpenApiParser(String fileContent, {bool isYaml = false}) {
    _definitionFileContent = isYaml ? (loadYaml(fileContent) as YamlMap).toMap() : jsonDecode(fileContent) as Map<String, dynamic>;
    ...
    if (_definitionFileContent.containsKey(_swaggerVar) && _definitionFileContent[_swaggerVar].toString().startsWith('2.0')) {
  ...
  Iterable<UniversalRestClient> parseRestClients() {
    ...
    UniversalType? returnTypeV3(Map<String, dynamic> map) {
      ...
      if (contentTypeValue.isEmpty || !contentTypeValue.containsKey(_schemaVar) || (contentTypeValue[_schemaVar] as Map<String, dynamic>).isEmpty) {
        return null;
      }
    ...
    List<UniversalRequestType> parametersV3(Map<String, dynamic> map) {
      ...
      if (map.containsKey(_parametersVar)) {
        for (var rawParameter in map[_parametersVar] as List<dynamic>) {
          ...
          final isRequired = (rawParameter as Map<String, dynamic>)[_requiredVar]?.toString().toBool();
          final typeWithImport = _findType(
    ...
    (_definitionFileContent[_pathsVar] as Map<String, dynamic>).forEach((path, pathValue) {
      ...
      pathValueMap.forEach((key, requestPath) {
        ...
        if (requestPath.containsKey(_parametersVar)) {
          requestPath[_parametersVar] = []
            ..addAll(requestPath[_parametersVar])
            ..addAll(pathParameters);
        } else {
          requestPath[_parametersVar] = pathParameters;
        }
        final requestPathResponses = (requestPath as Map<String, dynamic>)[_responsesVar] as Map<String, dynamic>;
        final returnType = _version == OpenApiVersion.v2 ? returnTypeV2(requestPathResponses) : returnTypeV3(requestPathResponses);
        final parameters = _version == OpenApiVersion.v2 ? parametersV2(requestPath) : parametersV3(requestPath);
        final requestName = requestPath[_operationIdVar]?.toString().toCamel ?? (key + path).toCamel;
  ...
  Iterable<UniversalDataClass> parseDataClasses() {
    ...
    if (_version == OpenApiVersion.v3_1 || _version == OpenApiVersion.v3) {
      if (!_definitionFileContent.containsKey(_componentsVar) || !(_definitionFileContent[_componentsVar] as Map<String, dynamic>).containsKey(_schemasVar)) {
        return dataClasses;
      }
      entities = (_definitionFileContent[_componentsVar] as Map<String, dynamic>)[_schemasVar] as Map<String, dynamic>;
    ...
    entities.forEach((key, value) {
      ...
      void findParamsAndImports(Map<String, dynamic> map) {
        List<String> erp = <String>[];
        if ((map as Map<String, dynamic>).containsKey(_requiredVar)) {
          erp = (map[_requiredVar] as List<dynamic>).map((e) => e.toString()).toList();
        }
        (map[_propertiesVar] as Map<String, dynamic>).forEach(
          (propertyName, propertyValue) {
            final typeWithImport = _findType(
              ...
              name: propertyName,
              isRequired: (requiredParameters..addAll(erp)).contains(propertyName),
            );
    ...
    // check for `allOf`
    final allOfClasses = dataClasses.where((dc) => dc is UniversalComponentClass && dc.allOf != null);
    ...
    // bug: if some classes import typedef class, they need import typedef class's import
    for (final def in dataClasses) {
      if (def is UniversalComponentClass) {
        if (def.typeDef) {
          for (final ref in dataClasses) {
            if (ref is UniversalComponentClass && ref.imports.contains(def.name.toPascal)) {
              ref.imports.addAll(def.imports);
            }
          }
        }
      }
  ...
  /// Get tag for name
  String _getTag(Map<String, dynamic> map) => map.containsKey(_tagsVar) ? (map[_tagsVar] as List<dynamic>).first.toString() : _defaultClientTag;
  ...
  String? _defaultValueCheck(Map<String, dynamic> map) => map.containsKey(_defaultVar) ? map[_defaultVar].toString() : null;
    ...
    if (map.containsKey(_typeVar) && map[_typeVar] == _arrayVar) {
      ...
      return TypeWithImport(
        ...
        import: newName,
            ...
            (map.containsKey(_propertiesVar) && (map[_propertiesVar] as Map<String, dynamic>).isNotEmpty) ||
        (map.containsKey(_additionalPropertiesVar) && (map[_additionalPropertiesVar] as Map<String, dynamic>).isNotEmpty)) {
      ...
      final type = map.containsKey(_typeVar)
          ? map.containsKey(_refVar) && map[_typeVar].toString() == _objectVar
              ? _formatRef(
                  useSchema ? (map[_schemaVar] as Map<String, dynamic>)[_refVar].toString() : map[_refVar].toString(),
                )
          ...
          : map.containsKey(_anyOfVar) || map.containsKey(_oneOfVar) || allOfObject
              ...
              : map.containsKey(_refVar)
                  ...
                  : _objectVar;
      final import = map.containsKey(_typeVar) || map.containsKey(_anyOfVar) || map.containsKey(_oneOfVar) || allOfObject
          ? null
          : map.containsKey(_refVar)
              ? _formatRef(
                  useSchema ? (map[_schemaVar] as Map<String, dynamic>)[_refVar].toString() : map[_refVar].toString(),
              ...
              : null;
      final nullable = root && map.containsKey(_nullableVar) && map[_nullableVar]!.toString().toBool();
  ...
  Map<String, dynamic> toMap() {
    ...
    for (final entry in entries) {
      if (entry.value is YamlMap || entry.value is Map) {
        ...
        map[entry.key.toString()] = (entry.value as YamlList).map<dynamic>((e) => e is YamlMap ? e.toMap() : e).toList(growable: false);
      } else {
